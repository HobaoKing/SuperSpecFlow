...
# 与 enabled 对称防御：disabled 是目录等非普通文件时 rm -f 会以原始系统错误退出，
# 这里改成可读错误信息，避免调用方看到裸 errno。
if [ -e "$project/.superspecflow/disabled" ] && [ ! -f "$project/.superspecflow/disabled" ]; then
  echo "error: $project/.superspecflow/disabled exists but is not a regular file" >&2
  exit 1
fi
...
- .superspecflow/enabled 仅为显式确认记录，不参与启用判定：SessionStart hook 与三份全局规则只依据 .superspecflow/disabled 判断是否禁用。
