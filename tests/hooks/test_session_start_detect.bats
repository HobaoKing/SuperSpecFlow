...
@test "cwd 默认（无 .superspecflow/disabled）时 additionalContext 为 enabled 标签" {
  cd "$PROJECT"
  run "$HOOK"
  [ "$status" -eq 0 ]
...
@test "cwd 有 .superspecflow/disabled 时 additionalContext 为 disabled 标签" {
...
@test "stdin JSON 中的 cwd 字段优先于 CLAUDE_PROJECT_DIR 和 PWD" {
  mkdir -p "$PROJECT/.superspecflow"
  touch "$PROJECT/.superspecflow/disabled"
  # PWD 与 CLAUDE_PROJECT_DIR 均指向未禁用的临时目录
  OTHER="$(ssf_make_tmp_project)"
  cd "$OTHER"
...
@test "stdin JSON 顶层 cwd 优先于嵌套对象中的 cwd（防止 sed 贪婪回归）" {
  # 顶层 cwd 指向 disabled 项目；payload 后续嵌套对象的 cwd 指向默认开启项目
  mkdir -p "$PROJECT/.superspecflow"
  touch "$PROJECT/.superspecflow/disabled"
...
@test "stdin JSON cwd 路径含空格时能正确解析" {
  ...
  touch "$SPACED/.superspecflow/disabled"
