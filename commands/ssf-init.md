...
定位包根目录：依次使用 `SUPERSPECFLOW_HOME`、`~/.claude/superspecflow/pack-root`、`~/.codex/superspecflow/pack-root`、`~/.gemini/superspecflow/pack-root`；当前目录确为包源码时可直接使用。只有无法定位时询问安装路径。
